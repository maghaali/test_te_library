...
class Sigmoid:
    ...
    def __init__(self, csv_file, current_supply, zoom):
        super(Sigmoid, self).__init__()
        df_data = pd.read_csv(csv_file[-1])
        self.l = df_data['l'][0]
        self.s = df_data['s'][0]
        self.m = df_data['m'][0]
        self.k = df_data['k'][0]
    ...
    def fig_builder(self):
        ...
        fig = go.Figure()
        fig.add_trace(
            go.Scatter(x=chart_x, y=chart_y, mode='lines', line_color='blue', hovertemplate = 'Supply:%{x:.2f}<br>Price: %{y:$.2f}', showlegend=False, name='Fund')
            )
        fig.add_trace(
            go.Scatter(x=area_x, y=area_y, fill='tozeroy', mode='lines', line_color='blue', hoverinfo='skip', showlegend=False)
            )
        fig.update_layout(barmode='stack', margin=dict(l=0, r=0, t=0, b=0), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
...
class MultiSigmoid(Sigmoid):
    ...
    def __init__(self, csv_file, current_supply, zoom):
        super(MultiSigmoid, self).__init__(csv_file, current_supply, zoom)
        df_data = pd.read_csv(csv_file[-1])
...
class Augmented(MultiSigmoid):
    ...
    """
    def __init__(self, csv_file, current_supply, zoom, reserve_rate_input):
...
class Smart(Augmented):
    ...
    def __init__(self, csv_file, current_supply, zoom, reserve_rate_input, reserve_power_input):
